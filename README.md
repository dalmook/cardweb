...
`admin.html`의 `삭제 관리` 영역에서 토픽 폴더의 음성 파일을 삭제할 수 있습니다.

1. GitHub token을 입력합니다.
2. 삭제할 토픽 폴더명을 입력하고 `파일 조회`를 누릅니다.
3. 일부 파일만 지우려면 파일을 선택한 뒤 `선택 파일 삭제`를 누릅니다.
4. 토픽 폴더의 음성 파일을 모두 지우려면 `토픽 폴더 전체 삭제`를 누릅니다.
5. 삭제 커밋 후 GitHub Actions가 `data/audio-topics.json`을 다시 갱신합니다.

GitHub는 빈 폴더를 저장하지 않으므로 토픽 폴더 안의 파일을 모두 삭제하면 저장소에서 해당 폴더도 사라진 것처럼 보입니다.
...
- 재생 간격: 기본 5초
- 답변시간: 기본 2분
- 전체시험시간: 기본 40분, `시작`/`종료` 버튼으로 별도 제어
