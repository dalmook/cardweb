...
2. `토픽 목록 조회`를 눌러 `audio/` 아래 토픽 폴더 목록을 불러옵니다.
3. 토픽 폴더 드롭다운에서 삭제할 파일이 있는 폴더를 선택합니다.
4. 파일 드롭다운에서 삭제할 음성 파일을 선택하고 `선택 파일 삭제`를 누릅니다.
5. 삭제 커밋 후 GitHub Actions가 `data/audio-topics.json`을 다시 갱신합니다.

안전을 위해 토픽 폴더 전체 삭제 버튼은 제공하지 않습니다.
